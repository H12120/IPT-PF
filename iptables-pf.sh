...
Set_forwarding_port(){
	read -e -p "请输入iptables转发的目的端口[1-65535](支持端口段:如 2333-6666):" forwarding_port
	[[ -z "${forwarding_port}" ]] && echo "取消..." && exit 1
	echo && echo -e "转发的目的端口: ${Red_font_prefix}${forwarding_port}${Font_color_suffix}" && echo
}
Set_forwarding_ip(){
		read -e -p "请输入iptables转发的目的IP:" forwarding_ip
		[[ -z "${forwarding_ip}" ]] && echo "取消..." && exit 1
		echo && echo -e "转发的目的IP : ${Red_font_prefix}${forwarding_ip}${Font_color_suffix}" && echo
}
Set_local_port(){
	echo -e "请输入iptables本地监听端口[1-65535](支持端口段 如 2333-6666)"
...
Set_local_ip(){
	read -e -p "请输入本服务器的网卡绑定IP(注意:可能是内网IP)(回车自动检测外网IP):" local_ip
	if [[ -z "${local_ip}" ]]; then
...
Add_forwarding(){
	...
	本地监听端口    : ${Green_font_prefix}${local_port}${Font_color_suffix}
	服务器 IP\t: ${Green_font_prefix}${local_ip}${Font_color_suffix}\n
	转发的目的端口    : ${Green_font_prefix}${forwarding_port_1}${Font_color_suffix}
	转发的目的IP\t: ${Green_font_prefix}${forwarding_ip}${Font_color_suffix}
	转发类型\t: ${Green_font_prefix}${forwarding_type}${Font_color_suffix}
——————————————————————————————\n"
}
View_forwarding(){
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/H12120/IPT-PF/main/iptables-pf.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1)
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	wget -N --no-check-certificate "https://raw.githubusercontent.com/H12120/IPT-PF/main/iptables-pf.sh" && chmod +x iptables-pf.sh
	echo -e "脚本已更新为最新版本[ ${sh_new_ver} ] !(注意：因为更新方式为直接覆盖当前运行的脚本，所以可能下面会提示一些报错，无视即可)" && exit 0
}
check_sys
echo && echo -e " iptables 端口转发一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  -- HLJONE | IPT-PF/PREFIX --
